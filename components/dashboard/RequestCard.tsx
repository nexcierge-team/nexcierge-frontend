...
const STATUS_STYLES: Record<RequestStatus, string> = {
  "Awaiting Specifications": "bg-zinc-100 text-zinc-700 ring-zinc-200",
  "Supplier Matching": "bg-blue-50 text-[#0066cc] ring-blue-100",
...
export function RequestCard({ request }: RequestCardProps) {
  return (
    <button className="group block w-full rounded-2xl border border-zinc-200 bg-white p-5 text-left transition-all duration-200 hover:border-zinc-300 hover:shadow-[0_4px_16px_-8px_rgba(0,0,0,0.08)]">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="text-xs font-medium tracking-[0.14em] text-zinc-400">
            {request.id}
          </div>
          <div className="mt-1.5 text-base font-semibold text-zinc-900">
            {request.title}
          </div>
          <div className="mt-0.5 text-sm text-zinc-500">{request.category}</div>
        </div>
      ...
      <div className="mt-5 flex items-center justify-between border-t border-zinc-100 pt-4 text-xs text-zinc-500">
        ...
        <ArrowUpRight
          className="h-4 w-4 text-zinc-300 transition-all duration-200 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-zinc-900"
          strokeWidth={1.5}
        />
      </div>
    </button>
