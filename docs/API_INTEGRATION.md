...
| `POST /api/am/sessions/[id]/translate-brief` | `POST /translate` (per uncached field) | AM dashboard — render the "Brief details" sidebar's free-text `rfqs` fields (`machine_type`, `intended_application`, `additional_notes`, `technical_specifications` values) in the AM's chosen working language, caching results in `rfqs.translations[lang]`. `compliance_requirements` and enum fields (`purchase_timeline`, `new_or_used_preference`) are excluded — enums resolve via the static `cardStrings.ts` lookup tables instead of a Gemini call |
...
### AM brief-sidebar translation (buyer → AM)

`POST /api/am/sessions/[id]/translate-brief` body `{ language: "zh" | "hi" }` renders the "Brief details" sidebar's free-text `rfqs` fields — `machine_type`, `intended_application`, `additional_notes`, and each `technical_specifications` value — in the AM's chosen working language. Same gating/rate-limit pattern as the message-translate route (`requireAccountManager`, `am-translate-brief:user:<id>` 60/min). `compliance_requirements` (certification codes like "CE"/"ISO 9001") and the two enum fields (`purchase_timeline`, `new_or_used_preference`) are deliberately excluded — the enums are already validated against fixed English sets server-side and have full label tables in `lib/cardStrings.ts`, so the sidebar resolves their display value via that lookup instead of a Gemini call.

For each candidate field: skip if a cached translation already exists in `rfqs.translations[lang]`; skip if `session.language === language` (the field is already authored in the target language — free-text profile fields follow the buyer's conversation language by design, see backend `docs/PROMPTS.md` → "Output language"); otherwise translate via `translateText` (chunked, bounded concurrency) and skip caching a no-op (the model echoing the input back verbatim). New translations persist to `rfqs.translations[lang]` via the **service-role admin client** — `rfqs_update_assigned_am` (migration `0008`) only lets the *assigned* AM update a row, but any AM should be able to preview a translated brief before claiming it.

Response: `{ language, translations: { machine_type?, intended_application?, additional_notes?, technical_specifications?: Record<string,string> } }` — only fields that actually needed translation, same "absent = show original" contract as the message-translate route. `BriefSummary` (`app/dashboard/page.tsx`) resolves each value from this cache when present.
