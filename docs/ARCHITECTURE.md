...
**AM display language (buyer/AI/AM → AM).** Separately, the AM dashboard lets the AM read the whole brief — chat thread AND the "Brief details" sidebar — in a chosen working language (`zh`/`hi`, or "Original only"; the set lives in `lib/amLanguages.ts`). `POST /api/am/sessions/[id]/translate` translates each message into that language on demand and caches the result in `chat_messages.metadata.translations[lang]` (admin client; never re-translated). `MessageBubble` in the AM view shows the original as primary and the translation below — but only when there IS one: a message already in the working language (e.g. the AM's own `zh` reply while reading in `zh`) is echoed verbatim by the backend and skipped, so it renders with no redundant secondary line.

`POST /api/am/sessions/[id]/translate-brief` does the equivalent for the sidebar's free-text `rfqs` fields (`machine_type`, `intended_application`, `additional_notes`, and each `technical_specifications` value — `compliance_requirements` and enum fields are excluded, see below), caching results on `rfqs.translations[lang]` (migration `0011`, same admin-client requirement — `rfqs_update_assigned_am` would otherwise block an AM from writing to a brief they haven't claimed). `BriefSummary` (`app/dashboard/page.tsx`) resolves each value from the cache when present, falling back to the original. `purchase_timeline`/`new_or_used_preference` are **not** sent through this route — they're already validated against fixed English enum sets server-side, so the AM sidebar resolves their display label via the same `cardStrings.ts` `timeline`/`condition` lookup tables the buyer card uses, no Gemini call needed. The sidebar's own chrome (section titles, field labels) reuses `cardStrings.ts` directly; the handful of AM-only strings with no buyer-card equivalent (the "Brief details" title, "CRM" section, status pill labels, HubSpot copy) live in `lib/amBriefStrings.ts`, scoped to just `en`/`zh`/`hi`.
