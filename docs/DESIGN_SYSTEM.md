...
- **Detail panel** (`BriefSummary`): white `w-80` right column (`border-l border-gray-200`). Header holds a `text-[15px] font-semibold` title and an inline status pill (no close control — the chat header's back arrow is the only way out of a brief). Sections use sentence-case `text-sm font-semibold` headings separated by `border-t border-gray-100` hairlines — optionally with an inline badge (the lead-quality pill sits next to "AI interview quality"). Field rows are a two-column grid: `grid-cols-[100px_1fr]`, label `text-gray-400` left, value `text-gray-800` wrapping right, `text-[13px]`. Empty fields are omitted, never placeholder'd.
